  ...
  /*
   * Adaptado por Eng. Carlos kwiek para tutorial no youtube e github.
   * Dezembro 2017
   * Link youtube https://www.youtube.com/watch?v=Iuj7BvXbYgY&list=PLDkRfSMHz1RiTlzfSbeZZcYf80G0sYl3m
   * Na descrição do video tem o link para o projeto completo no meu repositorio Github
  ...
  // Acima na parte comentada os links das biblotecas caso não as tenha instaladas.
  #include "WiFi.h"
  ...
  #define CANAL_ADC          34     //Aqui, define-se em qual canal ADC está ligado o potenciômetro (e, consequentemente, qual ADC será lido).
                                   //No ESP32 = 34, No NodeMCU, pode ser somente no 0. 
  ...
  #define DHTPIN                    16   //GPIO do NodeMCU ESP32 em que o sensor está ligado
...
#define TEMPO_CONFIRMACAO_MOVIMENTO                500    //tempo, em milisegundos, para confirmação de movimento detectado pelo sensor PIR
  ...
  const char* mqtt_server = "192.168.0.6";           //comente esta linha se quiser usar outro broker MQTT
 // const char* mqtt_server = "iot.eclipse.org";     //utilizado para testes / validação descomente esta linha se quiser usar este broker externo
 // const char* mqtt_server = "broker.hivemq.com"    //utilizado para testes / validação descomente esta linha se quiser usar este broker externo
  
  //constantes relacionadas a entradas e saidas
  const int buttonPin = 22;   //Pino em que estará ligado o botão
  const int ledPin = 5;       //Pino em que estará ligado o LED
  const int smart =  32;      //Pino que é acionado dependendo do setpoint, em relação a temperatura
  const int PIRPin = 33;      //Pino no qual estara ligado o sensor PIR 
  int value = touchRead(4);   //Pino ligado para acionar o botão touch ESP32
  ...
  */
  ...
  void verifica_e_envia_valor_potenciometro(void)
  {
   ...
   setpoint = map(setpoint, 0, 4095, 0, 60);
   float Temperatura;
...
void maquina_estados_sensor_PIR(void)
{
  ...
  void setup(void) {
      ...
      //inicializa temporizações de envio do ADC e temperatura e umidade
      TemporizacaoADC = millis();
      TemporizacaoTempUmid = millis();
